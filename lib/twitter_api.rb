      ...
      tweet2 = (EMOJI_REGEX.match(tweet.text)).to_s
        tweet2.split(" ").each do |word|
          if @emoji.join("").include?(word)
            if word_cloud[word]
              word_cloud[word][0] += 1
              if word_cloud[word][3].nil? && tweet.attrs[:coordinates]
                word_cloud[word][1] = tweet.text
                word_cloud[word][2] = tweet.attrs[:user][:location]
                word_cloud[word][3] = tweet.attrs[:coordinates][:coordinates]
                word_cloud[word][4] = tweet.attrs[:user][:screen_name]
              end
            else
              coord = tweet.attrs[:coordinates] ? tweet.attrs[:coordinates][:coordinates] : nil
              word_cloud[word] = [1, tweet.text, tweet.attrs[:user][:location], coord, tweet.attrs[:user][:screen_name]]
            end
          end
        end
    end
    word_cloud.keys.each do |key|
      if word_cloud[key][3].nil?
        word_cloud[key][3] = coordinates[rand(coordinates.length) - 1]
      end
      word_cloud[key][3][0], word_cloud[key][3][1] = word_cloud[key][3][1], word_cloud[key][3][0] if word_cloud[key][3]
    end
    puts word_cloud
    return word_cloud
  end
